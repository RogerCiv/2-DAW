...
class TicketsRepository{
    ...
    public static function cerrarTicket($idTicket){
        $bd = Conectar::conexion();
        $sql = "UPDATE tickets SET status = 1 WHERE id = $idTicket";
        $bd->query($sql);
    }
    ...
    public static function getAllTicketsByIdWhitResponse($userId){
        ...
        WHERE t.user_id = $userId AND t.id_trabajador != 0 AND t.status = 0";
    ...
    public static function getAllTicketsClosed($userId){
        ...
        FROM tickets t
        LEFT JOIN responses r ON t.id = r.ticket_id
        WHERE t.user_id = $userId AND t.id_trabajador != 0 AND t.status = 1";
    ...
    public static function getAllTicketsWhitT($idTrabajador){
        ...
        $sql = "SELECT * FROM tickets WHERE id_trabajador = $idTrabajador AND status = 0";
