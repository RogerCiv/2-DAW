...
function handlerInsertBook(e) {
  ...
  newBook.price = price.value;

  // Comprobar si el precio del libro es válido
  if (isNaN(newBook._price) || newBook._price <= 0) {
  ...
  const existingBook = getBooks.find(book => book._title.toLowerCase() === newBook._title.toLowerCase());
